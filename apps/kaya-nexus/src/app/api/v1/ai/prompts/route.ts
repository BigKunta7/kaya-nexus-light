...
import { aiPromptSchema } from '@/modules/ai/schema';
...
export const POST = requireRole(['user', 'admin'], async (req: NextRequest) => {
  ...
  const parse = aiPromptSchema.safeParse(body);
  if (!parse.success) {
    return NextResponse.json({ errors: parse.error.flatten().fieldErrors }, { status: 400 });
  }
  const supabase = createClient();
  const { data, error } = await supabase.from('ai_prompts').insert([parse.data]).select();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
